...
function label(page: PDFPage, text: string, x: number, y: number, bold: PDFFont) {
  page.drawText(text.toUpperCase(), { x, y, font: bold, size: 8, color: CORAL });
}
...
export async function buildTreatmentPdf(treatment: TreatmentData) {
  ...
  addBrand(cover, bold);
  label(cover, "30-second advertising treatment", MARGIN, 430, bold);
  ...
  treatment.generation.shots.forEach((shot, index) => {
    ...
    label(page, `Shot ${String(shot.shotNumber).padStart(2, "0")} / ${shot.purpose}`, MARGIN, 528, bold);

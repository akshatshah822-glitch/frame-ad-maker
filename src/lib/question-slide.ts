...
export async function createQuestionSlide(solve: QuestionSolve, state: QuestionSlideState = {}) {
  const width = 1920;
  const height = 1080;
  const questionLines = wrap(solve.question, 62).slice(0, 3);
  if (wrap(solve.question, 62).length > 3) throw new Error("Invalid solve.question: does not fit on the fixed slide.");
  ...
  const svg = `<svg width="${width}" height="${height}" xmlns="http://www.w3.org/2000/svg">
    ...
    ${textLines(questionLines, 150, 190, 52, 66, "#101b36", 800)}
    ${optionCards}
    ${countdown}

...
export function TreatmentView({ treatment, phase = "storyboard_ready", saved = true, currentShot, progressStep, progressElapsed = 0, onRetryShot, onRestart, initialVideoProduction = null }: Props) {
  ...
  return <main className="page result-page">
    ...
    <section className={`completion-state ${filmReady ? "is-ready" : "is-working"}`} aria-live="polite" aria-busy={!filmReady}>
      ...
      {filmReady ? <div className="completion-actions-wrap"><button className="primary-button" type="button" onClick={playFilm}>Watch AI preview <span aria-hidden="true">↓</span></button><CompletionActions treatment={treatment} onRestart={onRestart} filmReady={filmReady} canDownloadVideo={totalFrames === 6 && availableImageCount === 6} /></div> : null}
    </section>
    ...
    {filmReady ? <footer className="result-footer"><p>{saved ? "Treatment, frames, and AI preview saved." : "AI preview created. Saving was unavailable for this run."}</p></footer> : null}
  </main>;
}

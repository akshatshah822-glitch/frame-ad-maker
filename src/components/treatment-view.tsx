...
export function TreatmentView({ treatment, phase = "storyboard_ready", saved = true, currentShot, progressStep, progressElapsed = 0, onRetryShot, onRestart, initialVideoProduction = null }: Props) {
  ...
  const statusTitle = filmReady ? "AI ANIMATIC PREVIEW READY" : framesReady && !videoProduction ? "STORYBOARD READY" : framesReady ? `RENDERING ${renderedClips} OF 6` : incomplete || failedImageLoads.size ? `STORYBOARD INCOMPLETE · ${loadedCount}/${totalFrames} FRAMES` : phase === "images_generating" || loadingImages ? `GENERATING ${loadedCount} OF ${totalFrames}` : phase === "storyboard_generating" ? "DEVELOPING THE VISUAL WORLD" : "BUILDING THE STORYBOARD";
  const statusCopy = filmReady ? "Your AI animatic preview is ready to watch and share." : framesReady && !videoProduction ? "All six frames are ready. Start AI animatic preview production when you’re ready." : framesReady ? "Each generated shot is saved as it lands. The AI animatic preview player will appear here automatically." : incomplete || failedImageLoads.size ? `${failedCount} ${failedCount === 1 ? "frame needs" : "frames need"} attention. Retry only the affected frame below.` : `${progressStep || (currentShot ? `Drawing frame ${currentShot} of ${totalFrames}` : "Loading the storyboard frames")} · ${progressElapsed}s elapsed`;
  const playFilm = () => {
  ...
  return <main className="page result-page">
    ...
    <section className={`completion-state ${filmReady ? "is-ready" : "is-working"}`} aria-live="polite" aria-busy={!filmReady}>
      ...
      {filmReady ? <div className="completion-actions-wrap"><button className="primary-button" type="button" onClick={playFilm}>Watch AI animatic preview <span aria-hidden="true">↓</span></button><CompletionActions treatment={treatment} onRestart={onRestart} filmReady={filmReady} canDownloadVideo={totalFrames === 6 && availableImageCount === 6} /></div> : null}
    </section>
    ...
    {filmReady ? <footer className="result-footer"><p>{saved ? "Treatment, frames, and AI animatic preview saved." : "AI animatic preview created. Saving was unavailable for this run."}</p></footer> : null}
  </main>;
}
